...
**3 continuous daemons NOT scheduled** (no `--once` support — require code modification):
- `tier1_execution_daemon.py` — `while not shutdown_requested` loop
- `pre_tier_scoring_daemon.py` — `while True` loop
- `hypothesis_death_daemon.py` — `while True` loop
...
**Last Updated:** 2026-01-30 19:45 CET
**Author:** STIG (CTO)
