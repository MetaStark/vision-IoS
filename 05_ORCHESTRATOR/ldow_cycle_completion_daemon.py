...
def get_current_damper_hash(conn) -> str:
    """Get current damper configuration hash from LDOW freeze lock"""
    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        # Primary: Get from task_activation_status (LDOW freeze)
        cur.execute("""
            SELECT extended_config->'ldow_freeze'->>'damper_hash_at_lock' as damper_hash
        ...
        """)
        result = cur.fetchone()
        if result and result.get('damper_hash'):
            return result['damper_hash']

        # Fallback: Get from ldow_cycle_completion
        cur.execute("""
            SELECT damper_hash_at_start
        ...
        """)
        result = cur.fetchone()
        if result and result.get('damper_hash_at_start'):
            return result['damper_hash_at_start']

        # Last resort: Return the known locked hash from CEO-DIR-2026-065
        return 'da311f0ebb875122'
