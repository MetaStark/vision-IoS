# =====================================================================
# FHQ_MECHANISM_ALPHA_OUTCOME - Production Outcome Evaluator
# =====================================================================
# Pipeline step 3b: Evaluates open triggers past deadline, writes outcomes
# to fhq_learning.outcome_ledger. Feeds promotion gate.
#
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\mechanism_alpha_outcome.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("mechanism_alpha_outcome_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_MECHANISM_ALPHA_OUTCOME START" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -RedirectStandardOutput (Join-Path $LogDir "alpha_outcome_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "alpha_outcome_stderr.tmp")
    $exitCode = $process.ExitCode
    if (Test-Path (Join-Path $LogDir "alpha_outcome_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "alpha_outcome_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "alpha_outcome_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "alpha_outcome_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "alpha_outcome_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "alpha_outcome_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_MECHANISM_ALPHA_OUTCOME END (exit=$exitCode, ${duration}s)" | Out-File $LogFile -Append -Encoding utf8
...
VALUES ('FHQ_MECHANISM_ALPHA_OUTCOME',
 ...
 (SELECT jsonb_build_object('outcomes_today', (SELECT COUNT(*) FROM fhq_learning.outcome_ledger WHERE DATE(created_at) = CURRENT_DATE))),
 (SELECT jsonb_build_object('latest_outcome', (SELECT MAX(created_at)::text FROM fhq_learning.outcome_ledger))));
"@
