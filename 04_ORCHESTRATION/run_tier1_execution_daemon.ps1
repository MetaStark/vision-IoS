# =====================================================================
# FHQ_TIER1_EXECUTION_DAEMON - Continuous Hypothesis Tier 1 Evaluator
# =====================================================================
# Evaluates DRAFT hypotheses via price data, assigns tier1_result.
# Runs continuously with 30-minute internal sleep cycle.
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\tier1_execution_daemon.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("tier1_execution_daemon_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_TIER1_EXECUTION_DAEMON START (continuous)" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -RedirectStandardOutput (Join-Path $LogDir "tier1_exec_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "tier1_exec_stderr.tmp")
    $exitCode = $process.ExitCode
    if (Test-Path (Join-Path $LogDir "tier1_exec_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "tier1_exec_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "tier1_exec_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "tier1_exec_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "tier1_exec_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "tier1_exec_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_TIER1_EXECUTION_DAEMON EXIT (exit=$exitCode, ${duration}s)" | Out-File $LogFile -Append -Encoding utf8
...
VALUES ('FHQ_TIER1_EXECUTION_DAEMON',
 ...
 (SELECT jsonb_build_object('tier1_evaluated_today', (SELECT COUNT(*) FROM fhq_learning.hypothesis_canon WHERE tier1_result IS NOT NULL AND DATE(last_updated_at) = CURRENT_DATE))),
 (SELECT jsonb_build_object('latest_tier1_update', (SELECT MAX(last_updated_at)::text FROM fhq_learning.hypothesis_canon WHERE tier1_result IS NOT NULL))));
"@
