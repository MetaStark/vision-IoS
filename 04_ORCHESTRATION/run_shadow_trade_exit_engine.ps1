# =====================================================================
# FHQ_SHADOW_TRADE_EXIT_ENGINE - Production Shadow Trade Exit Engine
# =====================================================================
# CEO-DIR-20260130-PIPELINE-COMPLETION-002 D1
#
# Runs shadow_trade_exit_engine.py with full audit trail.
# Writes execution record to fhq_monitoring.run_ledger.
# Designed for Windows Task Scheduler - no interactive dependencies.
#
# Pipeline step 5.1: after shadow trades created, evaluates exit conditions.
#
# Exit codes:
#   0 = Success
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\shadow_trade_exit_engine.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("shadow_trade_exit_engine_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
# --- Execute ---
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_SHADOW_TRADE_EXIT_ENGINE START" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -Wait `
        -PassThru `
        -RedirectStandardOutput (Join-Path $LogDir "shadow_exit_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "shadow_exit_stderr.tmp")

    $exitCode = $process.ExitCode

    # Append stdout/stderr to log
    if (Test-Path (Join-Path $LogDir "shadow_exit_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "shadow_exit_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "shadow_exit_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "shadow_exit_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "shadow_exit_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "shadow_exit_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_SHADOW_TRADE_EXIT_ENGINE END" | Out-File $LogFile -Append -Encoding utf8
...
('FHQ_SHADOW_TRADE_EXIT_ENGINE',
 ...
 (SELECT jsonb_build_object(
    'shadow_trades_closed', (SELECT COUNT(*) FROM fhq_execution.shadow_trades WHERE status = 'CLOSED' AND DATE(updated_at) = CURRENT_DATE)
 )),
 (SELECT jsonb_build_object(
    'latest_exit_time', (SELECT MAX(exit_time)::text FROM fhq_execution.shadow_trades WHERE status = 'CLOSED')
 ))
);
"@
