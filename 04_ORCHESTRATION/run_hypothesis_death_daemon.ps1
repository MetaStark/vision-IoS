# =====================================================================
# FHQ_HYPOTHESIS_DEATH_DAEMON - Continuous Hypothesis Expiry Enforcer
# =====================================================================
# Falsifies hypotheses past expected_timeframe_hours.
# Runs continuously with 15-minute internal sleep cycle.
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\hypothesis_death_daemon.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("hypothesis_death_daemon_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_HYPOTHESIS_DEATH_DAEMON START (continuous)" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -RedirectStandardOutput (Join-Path $LogDir "hyp_death_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "hyp_death_stderr.tmp")
    $exitCode = $process.ExitCode
    if (Test-Path (Join-Path $LogDir "hyp_death_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "hyp_death_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "hyp_death_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "hyp_death_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "hyp_death_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "hyp_death_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_HYPOTHESIS_DEATH_DAEMON EXIT (exit=$exitCode, ${duration}s)" | Out-File $LogFile -Append -Encoding utf8
...
VALUES ('FHQ_HYPOTHESIS_DEATH_DAEMON',
 ...
 (SELECT jsonb_build_object('falsified_today', (SELECT COUNT(*) FROM fhq_learning.hypothesis_canon WHERE status = 'FALSIFIED' AND DATE(falsified_at) = CURRENT_DATE))),
 (SELECT jsonb_build_object('latest_falsified', (SELECT MAX(falsified_at)::text FROM fhq_learning.hypothesis_canon WHERE status = 'FALSIFIED'))));
"@
