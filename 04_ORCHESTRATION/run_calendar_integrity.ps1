# =====================================================================
# FHQ_CALENDAR_INTEGRITY - Daily Calendar Integrity Check
# =====================================================================
# CEO-DIR-2026-091: Asserts forward calendar coverage >= 720 days,
# validates next 7 trading days resolve cleanly.
#
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\calendar_integrity_daemon.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("calendar_integrity_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_CALENDAR_INTEGRITY START" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -RedirectStandardOutput (Join-Path $LogDir "cal_integrity_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "cal_integrity_stderr.tmp")
    $exitCode = $process.ExitCode
    if (Test-Path (Join-Path $LogDir "cal_integrity_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "cal_integrity_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "cal_integrity_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "cal_integrity_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "cal_integrity_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "cal_integrity_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_CALENDAR_INTEGRITY END (exit=$exitCode, ${duration}s)" | Out-File $LogFile -Append -Encoding utf8
...
VALUES ('FHQ_CALENDAR_INTEGRITY',
 ...
 (SELECT jsonb_build_object('calendar_days_forward', (SELECT COUNT(*) FROM fhq_meta.calendar_days WHERE date >= CURRENT_DATE))),
 (SELECT jsonb_build_object('max_calendar_date', (SELECT MAX(date)::text FROM fhq_meta.calendar_days))));
"@
