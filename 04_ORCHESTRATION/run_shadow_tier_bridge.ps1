# =====================================================================
# FHQ_SHADOW_TIER_BRIDGE - Production Shadow Tier Bridge
# =====================================================================
# CEO-DIR-20260130-VERIFY-FOR-REAL-001 V4 follow-up
#
# Runs shadow_tier_bridge.py with full audit trail.
# Writes execution record to fhq_monitoring.run_ledger.
# Designed for Windows Task Scheduler - no interactive dependencies.
#
# Pipeline step 5: after promotion_gate_audit PASS, links to shadow trades.
#
# Exit codes:
#   0 = Success
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\shadow_tier_bridge.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("shadow_tier_bridge_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
# --- Execute ---
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_SHADOW_TIER_BRIDGE START" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -Wait `
        -PassThru `
        -RedirectStandardOutput (Join-Path $LogDir "shadow_bridge_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "shadow_bridge_stderr.tmp")

    $exitCode = $process.ExitCode

    # Append stdout/stderr to log
    if (Test-Path (Join-Path $LogDir "shadow_bridge_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "shadow_bridge_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "shadow_bridge_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "shadow_bridge_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "shadow_bridge_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "shadow_bridge_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_SHADOW_TIER_BRIDGE END" | Out-File $LogFile -Append -Encoding utf8
...
('FHQ_SHADOW_TIER_BRIDGE',
 ...
 (SELECT jsonb_build_object(
    'shadow_tier_registry', (SELECT COUNT(*) FROM fhq_learning.shadow_tier_registry)
 )),
 (SELECT jsonb_build_object(
    'latest_shadow_entry', (SELECT MAX(sampled_at)::text FROM fhq_learning.shadow_tier_registry)
 ))
);
"@
