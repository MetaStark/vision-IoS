# =====================================================================
# FHQ_SHADOW_TRADE_CREATOR - Production Shadow Trade Creator
# =====================================================================
# CEO-DIR-20260130-SHADOW-EXECUTION-BRIDGE-001 D2
#
# Runs shadow_trade_creator.py with full audit trail.
# Writes execution record to fhq_monitoring.run_ledger.
# Designed for Windows Task Scheduler - no interactive dependencies.
#
# Pipeline step 4.5: after trigger_events, before shadow_tier_bridge.
#
# Exit codes:
#   0 = Success
...
$ScriptPath  = Join-Path $RepoRoot "03_FUNCTIONS\shadow_trade_creator.py"
$LogDir      = Join-Path $RepoRoot "04_ORCHESTRATION\logs"
$LogFile     = Join-Path $LogDir ("shadow_trade_creator_{0}.log" -f (Get-Date -Format "yyyyMMdd_HHmmss"))
...
# --- Execute ---
$startTime = Get-Date
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_SHADOW_TRADE_CREATOR START" | Out-File $LogFile -Encoding utf8
...
try {
    $process = Start-Process -FilePath $PythonPath `
        ...
        -Wait `
        -PassThru `
        -RedirectStandardOutput (Join-Path $LogDir "shadow_creator_stdout.tmp") `
        -RedirectStandardError (Join-Path $LogDir "shadow_creator_stderr.tmp")

    $exitCode = $process.ExitCode

    # Append stdout/stderr to log
    if (Test-Path (Join-Path $LogDir "shadow_creator_stdout.tmp")) {
        Get-Content (Join-Path $LogDir "shadow_creator_stdout.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "shadow_creator_stdout.tmp") -ErrorAction SilentlyContinue
    }
    if (Test-Path (Join-Path $LogDir "shadow_creator_stderr.tmp")) {
        "--- STDERR ---" | Out-File $LogFile -Append -Encoding utf8
        Get-Content (Join-Path $LogDir "shadow_creator_stderr.tmp") | Out-File $LogFile -Append -Encoding utf8
        Remove-Item (Join-Path $LogDir "shadow_creator_stderr.tmp") -ErrorAction SilentlyContinue
    }
} catch {
...
"" | Out-File $LogFile -Append -Encoding utf8
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - FHQ_SHADOW_TRADE_CREATOR END" | Out-File $LogFile -Append -Encoding utf8
...
('FHQ_SHADOW_TRADE_CREATOR',
 ...
 (SELECT jsonb_build_object(
    'shadow_trades', (SELECT COUNT(*) FROM fhq_execution.shadow_trades WHERE DATE(created_at) = CURRENT_DATE)
 )),
 (SELECT jsonb_build_object(
    'latest_shadow_entry', (SELECT MAX(entry_time)::text FROM fhq_execution.shadow_trades)
 ))
);
"@
