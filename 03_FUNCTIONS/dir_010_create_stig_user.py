...
-- Create STIG user if not exists (workaround for psycopg2 permissions)
"""
...
def main():
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT 1 FROM pg_user WHERE usename = 'STIG'")
            exists = cur.fetchone()
            if not exists[0]:
                print("[DIR-010] STIG user does not exist, creating...")
                cur.execute("""
                    CREATE USER STIG
                          WITH PASSWORD E'STIG_DAEMON_PASSWORD_CHANGE_IT'
                          NOINHERIT
                          NOCREATEDB
                          LOGIN
                          SUPERUSER
