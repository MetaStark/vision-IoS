...
def run_at2(conn):
    ...
    # CEO-DIR-2026-015: Non-controlled must be BLOCKED during freeze
    if not allowed and "GENERATION_FREEZE" in reason:
        print("PASS: AT-2 (Non-controlled correctly blocked)")
        return True
    else:
        print(f"FAIL: AT-2 - Expected BLOCK for non-controlled")
        return False
...
def run_at3(conn):
    ...
    if allowed and "GENERATION_FREEZE: Controlled exception allowed" in reason:
        print("PASS: AT-3")
        return True
    else:
