    ...
    try:
        ...
        with conn.cursor() as cur:
            ...
            if diversity_result and not diversity_result[0]:  # allowed = False
                ...
                conn.commit()
                logger.warning(f"DIVERSITY_BLOCK: {diversity_result[4]} - semantic_hash={semantic_hash[:8]}... (error={error.get('error_code')})")
                conn.close()
                return None
        ...
        with conn.cursor() as cur:
            ...
            if quota_result and not quota_result[2]:  # scaling_allowed = False
                ...
                conn.commit()
                logger.warning(f"SCALING_BLOCK: {quota_result[3]} - semantic_hash={semantic_hash[:8]}... (error={error.get('error_code')})")
                conn.close()
                return None
        ...
        with conn.cursor() as cur:
            ...
            if structure_quota_result and not structure_quota_result[0]:  # allowed = False
                ...
                conn.commit()
                logger.warning(f"QUOTA_BLOCK: {structure_quota_result[3]} - semantic_hash={semantic_hash[:8]}... (error={error.get('error_code')})")
                conn.close()
                return None

        # CEO-DIR-2026-128: MEMORY BIRTH GATE
        # Query prior failures before hypothesis birth
        causal_mechanism = rationale['causal_mechanism']
        ...
        with conn.cursor() as cur:
            ...
            if memory_result and memory_result[4]:  # should_block = True
                ...
                conn.commit()
                logger.warning(f"MEMORY_BLOCK: {memory_result[5]} - prior_count={memory_result[0]} (error={error.get('error_code')})")
                conn.close()
                return None
            ...
            # FINN-E generates non-controlled hypotheses by default
        ...
        with conn.cursor() as cur:
            ...
            cur.execute("""
                INSERT INTO fhq_learning.hypothesis_canon (
                    ...
                    48, %s, %s, %s, 3, 0.55, 0.55, 'DRAFT', %s, 1, %s, NOW(), %s, %s, %s, %s
                )
